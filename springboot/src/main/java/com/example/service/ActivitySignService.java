 ...
 **/
@Service
public class ActivitySignService extends BaseService{
    ...
    public PageInfo<ActivitySign> selectPage(ActivitySign activitySign, Integer pageNum, Integer pageSize) {
        ...
        //
        Map<String, String> map = ParaConfig("switchname");
        if (map.get("PARACODE1").equals("true")) {
            List<ActivitySign> list1 = new ArrayList<>();
            //查询所有待审核的记录
            for (ActivitySign everlist : list) {
                boolean status = everlist.getStatus().equals("待审核");
                if (status){
                    list1.add(everlist);
                }
            }
